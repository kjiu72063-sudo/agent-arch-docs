...
Codex 的上下文治理偏**源头减量**——在内容**进入上下文之前**就把它压小，而不是"进来了再摘要"。把它放进 [2-2 决策三](/concepts/context/design) 的三层坐标系：

| 时机 | 做法 | Codex 对应 |
|---|---|---|
| ① 进上下文**之前**（源头） | 最小化控制台输出 / 日志落盘 / grep 友好格式 / 预计算聚合 | ✅ **主体策略** |
| ② 进了之后、摘要之前 | DSE 抽结构 + 约束原文保留 | 无专门机制，靠 `AGENTS.md` 纪律间接约束 |
| ③ 摘要 | LLM 凝练残余 | 依赖模型侧窗口管理，非 harness 显式策略 |

> **启示**：**最省的压缩是不让内容进上下文**。源头减量发生在 token 记账之前，收益比任何下游压缩都直接；但它只对"可预知会很长"的内容有效（命令输出、构建日志），对"用户说了什么"无效——后者仍要靠 ② / ③。

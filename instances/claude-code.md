...
它的实际做法属于 [2-2 决策三](/concepts/context/design) 里的 **`hybrid` 形态**（DSE 先抽结构，LLM 只凝练残余）：

| 内容 | 处理 | 对应策略 |
|---|---|---|
| 用户身份 / 文件路径 / 工具调用记录 | 保留为结构化记录 | **DSE**（可枚举、可断言） |
| 硬性约束（"不要改 lockfile"） | **原文保留**，不进入摘要 | **DSE + 原文保留** |
| 讨论过程 / 失败尝试 / 寒暄 | 交给 LLM 凝练成一段摘要 | **LLM 摘要**（只吃残余） |

> 之所以不做"全量 LLM 摘要"：约束和路径一旦被摘要**改写措辞**就会静默失效（顺序契约见 [2-2 决策三](/concepts/context/design)）。判断一个 agent 的压缩是否成熟，最快的方法是看它的约束是被**"保留"**还是被**"总结"**。

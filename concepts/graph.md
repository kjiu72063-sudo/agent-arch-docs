...
> **难度**：5-1 机制（入门→熟悉） · 5-2 设计决策（熟悉→精通） · 5-3 坑与自检（全档复核）
...
| [🧭 5-2 设计决策 + 验证](/concepts/graph/design) | reducer 三类对照、checkpointer 生产选型、**HITL 决策表**、**Send 并行归并**、**gate 验收脚本** | 设计 + 验证 |
| [⚠️ 5-3 常见坑 + 自检](/concepts/graph/pitfalls) | 滥用图、reducer 理解错、缺 checkpoint 等坑 + **产物化三档**（精通=提交含条件边的 StateGraph 并跑通） | 自检 |

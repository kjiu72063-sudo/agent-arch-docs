...
::: info 下一步：进入 Track A（难度接力）
到这里 Track 0 的知识已够支撑进入概念主轴。**第一个进入点是 [01 prompt engineering](/concepts/prompt)**（L0 入门）——它直接从 [0-3 对话角色](./03-roles) 的 system/user 分工接续，不跳级。

Track A 每章按 `机制 → 设计决策 → 坑与自检` 递进；我们建议：第一遍只读到"机制详解"（L1），跑通 gate 后再回头读 L2 的设计决策。这样不会一上来就被第三层的坑与决策吓退。
:::

> 上一步：[0-5 什么是 Agent](./05-agent) ｜ 回到：[Track 0 总入口](./) ｜ 进入：[Track A · 01 prompt](/concepts/prompt)
